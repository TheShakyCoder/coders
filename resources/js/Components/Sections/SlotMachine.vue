...
<template>
  <div id="builder" class="hero-panel">
    <div class="flex items-start justify-between gap-4">
      <div>
        <h2
          class="mb-4 font-display text-5xl uppercase tracking-[0.08em] text-stone-50 flex flex-col"
        >
    ...
    <div class="machine-shell" :class="{ 'jackpot-active': jackpot }">
      ...
      <!-- Slot progress dots -->
      <div
        class="flex items-center justify-center gap-2 border-b border-white/[0.03] bg-black/25 px-4 py-2"
      >
        <div
          v-for="j in boxLimit"
          :key="j"
          ...
          :style="
            selectedSlots[j - 1]
              ? {
                  backgroundColor: selectedSlots[j - 1].accent,
                  boxShadow: `0 0 8px ${selectedSlots[j - 1].accent}80`,
                }
              : {}
          "
        />
        ...
        <div class="flex flex-1 items-start px-2">
          <template v-for="(_, i) in slotIndices" :key="i">
            ...
            <div class="flex flex-1 flex-col items-center gap-[0.4rem] px-1.5">
              ...
              <!-- Reel viewport -->
              <div
                class="reel-viewport relative w-full overflow-hidden rounded-[14px] border bg-black/[0.82] shadow-[inset_0_2px_16px_rgba(0,0,0,0.8),inset_0_0_0_1px_rgba(255,255,255,0.03)] transition-[border-color] duration-500"
                :style="
                  getReelItem(i).in_stock === false
                    ? {
                        height: `${3 * 120}px`,
              ...
              >
                ...
                <!-- Win-line -->
                <div
                  class="pointer-events-none absolute inset-x-0 z-[11] border-b border-t transition-[border-color,background] duration-500"
                  :style="{
                    top: '120px',
                    height: '120px',
                    borderTopColor:
                    ...
                    background:
                      getReelItem(i).in_stock === false
                        ? 'rgba(239,68,68,0.08)'
                        : getReelItem(i).accent + '0d',
                  }"
                />

                <!-- Out of stock badge -->
                <div
                  v-if="!isSpinning && getReelItem(i).in_stock === false"
                  class="pointer-events-none absolute inset-x-0 z-[12] flex items-center justify-center"
                  style="top: 120px; height: 120px"
                >
                  <span
                    class="rounded-full border border-red-500/40 bg-black/70 px-2 py-0.5 font-display text-[0.55rem] uppercase tracking-[0.2em] text-red-400 backdrop-blur-sm"
                  >
                    Out of Stock
                  </span>
                </div>
    ...
    <div class="mt-6 rounded-[1.8rem] border border-white/10 bg-black/25 p-5">
      <div
        class="flex items-center justify-between text-sm uppercase tracking-[0.2em] text-stone-400"
      >
        <span>Box total</span>
      ...
      <p class="mt-3 text-sm leading-6 text-stone-400">{{ deliveryPreview }}.<br /></p>
    </div>

    <div
      v-if="hasOutOfStock"
      class="mt-4 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-300"
    >
    ...
    <div class="mt-6 flex flex-col gap-3 sm:flex-row">
      <button
        type="button"
        class="fire-button"
        :disabled="loadingCheckout || !isBoxFull || hasOutOfStock"
        @click="startCheckout"
      >
        {{ checkoutLabel }}
      </button>
...
.machine-shell {
  ...
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.06),
    inset 0 -6px 24px rgba(0, 0, 0, 0.7), 0 0 0 3px rgba(0, 0, 0, 0.5),
    0 0 0 4px rgba(100, 90, 80, 0.12), 0 12px 40px rgba(0, 0, 0, 0.55);
  overflow: hidden;
...
@keyframes jackpot-pulse {
  ...
  50% {
    box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.06),
      inset 0 -6px 24px rgba(0, 0, 0, 0.7), 0 0 0 3px rgba(0, 0, 0, 0.5),
      0 0 0 4px rgba(249, 115, 22, 0.5), 0 12px 60px rgba(249, 115, 22, 0.35),
...
@keyframes reel-btn-pulse {
  ...
  50% {
    ...
    box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.08), 0 0 18px rgba(34, 197, 94, 0.6),
...
@keyframes spin-btn-glow {
  0%,
  100% {
    box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.07), 0 0 12px rgba(249, 115, 22, 0.2),
      0 0 24px rgba(249, 115, 22, 0.08);
    border-color: rgba(249, 115, 22, 0.35);
  }
  50% {
    box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.07), 0 0 28px rgba(249, 115, 22, 0.55),
      0 0 52px rgba(249, 115, 22, 0.2);
...
.spin-random-btn:hover:not(:disabled) {
  ...
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.07), 0 0 36px rgba(249, 115, 22, 0.65),
    0 0 64px rgba(249, 115, 22, 0.25);
  animation: none;
}
</style>
