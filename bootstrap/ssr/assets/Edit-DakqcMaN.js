...
//#region resources/js/Pages/Admin/Products/Edit.vue
var _sfc_main = {
	__name: "Edit",
	...
	setup(__props) {
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<!--[-->`);
			_push(ssrRenderComponent(unref(Head), { title: `Edit ${__props.product.name}` }, null, _parent));
			_push(ssrRenderComponent(AdminLayout_default, null, {
				header: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<h2 class="font-bold text-2xl text-stone-100 uppercase tracking-tighter"${_scopeId}> Edit <span class="text-orange-500"${_scopeId}>${ssrInterpolate(__props.product.name)}</span></h2></div>`);
						...
						_: 1
					}, 8, ["href"]), createVNode("h2", { class: "font-bold text-2xl text-stone-100 uppercase tracking-tighter" }, [createTextVNode(" Edit "), createVNode("span", { class: "text-orange-500" }, toDisplayString(__props.product.name), 1)])])];
				}),
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
